...
def label_confidence_reward(
    ...
    **kwargs,
    ...
    - 正常性：judge=1表示正常点击，judge=0表示欺骗点击。
    - 匹配：根据types判断期望judge（1=benign期望judge=1，0=deceptive期望judge=0）。
    - 奖励：如果judge匹配期望，正奖励=conf；否则负奖励=-conf，并记录样本。
    """
    
    completion_texts = _extract_text_from_completions(completions)
    n = len(completion_texts)
        # 检查 snapshot_path 和 id 是否存在
    if snapshot_path and "id" in kwargs:
        pass 
    elif snapshot_path:
        print(f"⚠️[RWD] snapshot_path exists but 'id' not in kwargs! keys: {list(kwargs.keys())}")
        # 广播types到n长度（如果需要）
