...
def run_on_dataset(agent: Local, ann_path: str, images_dir: str, limit: int | None = None, out_dir: str | None = None, test_size=0.2, seed=42):
    ...
    for sample in ds:
        ...
        rec = {
            "id": sample.get("id") or sample.get("image_path") or count,
    ...
    pos = sum(1 for r in results if r.get("reward") is not None and r.get("reward") > 0)
    neg = sum(1 for r in results if r.get("reward") is not None and r.get("reward") < 0)
    parse_failures = sum(1 for r in results if r.get("judge") is None or r.get("conf") is None)

    avg_conf_overall = None
    if parsed_count > 0:
        avg_reward_overall = sum(r.get("reward", 0.0) for r in parsed) / parsed_count

    # avg conf for matches / mismatches
    matches = [r for r in results if r.get("reward") is not None and r.get("reward") > 0]
    mismatches = [r for r in results if r.get("reward") is not None and r.get("reward") < 0]
    ...
    parsed_by_expected = {}
    for L in (1, -1, 0):
        parsed_by_expected[L] = [r for r in parsed if r.get("expected_type") == L]
