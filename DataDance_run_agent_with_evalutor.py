...
DEFAULT_ADAPTER_DIR = "UI-TARS-1.5-7B-Click-25/checkpoint-800"
...
def get_evaluator(system_prompt: str = "") -> Local:
    ...
    model_path = DEFAULT_ADAPTER_DIR if os.path.exists(DEFAULT_ADAPTER_DIR) else None
    ...
    for idx, entry in enumerate(data[:max_samples]):
        ...
        for cat, cs in category_stats.items():
            ...
            per_category_metrics[cat] = {
                ...
                "TCR_eval": cs.get("num_task_completed_eval", 0.0) / n,
                "DFR_eval": cs.get("num_deceptive_fail_eval", 0.0) / n,
